...
\begin{frame}{Du Single-Linkage au Robust Single-Linkage}
    \begin{columns}[c, onlytextwidth]
        \begin{column}{0.55\textwidth}
            Le \textbf{Single-Linkage} classique ($1\text{-NN}$) souffre de l'\emph{effet de chaînage} : une seule observation de bruit peut fusionner deux amas distincts.
            
            \vspace{0.4cm}
            \textbf{Robust Single-Linkage} (Chaudhuri \& Dasgupta, 2010) :
            \begin{itemize}
                \setlength{\itemsep}{6pt}
                \item Substitue l'estimateur de densité $1\text{-NN}$ par le \textbf{$K\text{-NN}$}.
                \item \textbf{Distance de cœur} (\emph{core distance}) $r_K(x)$ :
                \[
                    r_K(x) = \min \bigl\{ r \ge 0 \bigm| |B(x, r) \cap \mathcal{X}| \ge K \bigr\}
                \]
                \item \textbf{Distance d'atteignabilité mutuelle} :
                \[
                    d_{\mathrm{mreach}, K}(x, y) = \max \bigl\{ r_K(x), r_K(y), d(x, y) \bigr\}
                \]
            \end{itemize}
        \end{column}
        ...
        \begin{column}{0.42\textwidth}
            \begin{block}{Algorithme}
                On applique le Single-Linkage (extraction de l'arbre couvrant minimum, MST) sur le graphe complet pondéré par la métrique $d_{\mathrm{mreach}, K}$.
            \end{block}
            
            \vspace{0.3cm}
            \begin{itemize}
                \setlength{\itemsep}{6pt}
                \item Deux points ne se connectent à une échelle $r$ que s'ils sont géométriquement proches \textbf{et} situés dans des régions de forte densité ($r_K(x), r_K(y) \le r$).
                \item Consistance mathématique robuste démontrée lorsque $K \to \infty$.
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Du Robust Single-Linkage à (H)DBSCAN}
    \begin{columns}[c, onlytextwidth]
        \begin{column}{0.52\textwidth}
            ...
            \begin{itemize}
                \setlength{\itemsep}{6pt}
                \item Trop exigeant pour les points frontières (qui n'ont pas $K$ voisins).
                \item Ceux-ci sont rejetés dans le bruit, ce qui nuit au rappel des clusters.
            \end{itemize}
            
            \vspace{0.3cm}
            \textbf{DBSCAN} (Ester \emph{et al.}, 1996) à un seuil $r$ :
            \begin{itemize}
                \setlength{\itemsep}{4pt}
                \item \textbf{Points cœurs} : $r_K(x) \le r$ (au moins $K$ voisins).
                \item \textbf{Points accessibles} (frontières) : $r_K(x) > r$, mais à distance $\le r$ d'un point cœur.
                \item \textbf{Bruit} : tous les autres points.
            \end{itemize}
        \end{column}
        
        \begin{column}{0.45\textwidth}
            \begin{block}{De DBSCAN à HDBSCAN}
                DBSCAN requiert un seuil global $r$ unique, inadapté aux densités variables.
                
                \vspace{0.2cm}
                \textbf{HDBSCAN} (Campello \emph{et al.}, 2013) :
                \begin{itemize}
                    \item Fait varier le seuil $r$ continûment pour construire une hiérarchie complète.
                    \item Utilise la distance d'atteignabilité mutuelle $d_{\mathrm{mreach}, K}$.
                    \item Extrait la hiérarchie simplifiée via l'excès de masse.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Du DBSCAN à HDBSCAN : l'excès de masse}
    \begin{columns}[c, onlytextwidth]
        \begin{column}{0.54\textwidth}
            \begin{center}
                \begin{tikzpicture}[scale=0.35, >=latex, font=\scriptsize]
                    ...
                    \draw[<-, shorten <=1pt, semithick] (1.876, 1.823) -- ++(-0.5, 0) node[left] {$C_1$};
                    \draw[<-, shorten <=1pt, semithick] (3.499, 3.103) -- ++(-0.5, 0) node[left] {$C_2$};
                    \draw[<-, shorten <=1pt, semithick] (2.327, 1.233) -- ++(-0.5, 0) node[left] {$C_4$};
                    \draw[<-, shorten <=1pt, semithick] (5.999, 1.781) -- ++(-0.5, 0) node[left] {$C_3$};
                    \draw[<-, shorten <=1pt, semithick] (4.899, 0.380) -- ++(-0.5, 0) node[left] {$C_5$};
                \end{tikzpicture}%
                \hfill
                \begin{tikzpicture}[scale=0.35, >=latex, font=\scriptsize]
                    \tikzset{declare function={f(\x) = 1.8*exp(-pow(\x-1.8, 2)/0.4) + 4.5*exp(-pow(\x-3.5, 2)/0.8) + 2.8*exp(-pow(\x-6.0, 2)/0.6);}}
        ...
        \begin{column}{0.44\textwidth}
            \textbf{Excès de masse} (\emph{excess of mass}) \cite{HDBSCAN} :
            \begin{itemize}
                \setlength{\itemsep}{4pt}
                \item Mesure la stabilité d'un cluster $C$ apparu à une densité $\lambda$ :
                \[ E(C) = \int_C \bigl(f(x) - \lambda\bigr) \mathbf{1}_{f(x) \ge \lambda} \, \mathrm{d}x \]
                \item Graphiquement, cela correspond aux \textbf{aires colorées} sous la courbe.
            \end{itemize}
            \vspace{0.2cm}
            \begin{block}{Simplification de l'arbre}
                De bas en haut dans l'arbre condensé :
                \begin{itemize}
                    \item Si $\sum \text{Stabilité}(Enfants) > \text{Stabilité}(Parent)$, on garde la scission.
                    \item Sinon, le parent absorbe les enfants.
                    \item Extraction finale optimale ($C_1, C_2, C_3$).
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Robust SL / (H)DBSCAN vs. Hiérarchie exacte 2-NN}
    \begin{columns}[c, onlytextwidth]
        \begin{column}{0.48\textwidth}
            \begin{center}
                \begin{tikzpicture}[scale=0.75]
                    ...
                    % Overlay 1: Robust SL / (H)DBSCAN (K=2) à r_mreach = 2r
                    \only<1>{
                        \foreach \P in {A,B,C,D,E,F} {
                            \fill[inria-bleu, opacity=0.15] (\P) circle (\r);
                            \draw[dashed, gray!60, thick] (\P) circle (\r);
                        }
                        \draw[line width=1.5pt, inria-rouge] (A) -- (B) -- (C) -- cycle;
                        \draw[line width=1.5pt, inria-rouge] (D) -- (E) -- (F) -- cycle;
                        \draw[line width=1.5pt, inria-rouge] (C) -- (D);
                    }

                    % Overlay 2: True 2-NN hierarchy (HGP-Clusterer) à r' = 2/\sqrt{3} r
                    \only<2>{
                        \foreach \P in {A,B,C,D,E,F} {
                        ...
                        \draw[line width=1.5pt, inria-2024-bleu-canard] (A) -- (B) -- (C) -- cycle;
                        \draw[line width=1.5pt, inria-2024-bleu-canard] (D) -- (E) -- (F) -- cycle;
                        \draw[line width=1.5pt, inria-2024-gris-bleu, dotted] (C) -- (D);
        ...
        \begin{column}{0.50\textwidth}
            \only<1>{
                \textbf{Robust SL / (H)DBSCAN ($K=2$)} :
                \begin{itemize}
                    \setlength{\itemsep}{8pt}
                    \item Seuil d'atteignabilité mutuelle $d_{\mathrm{mreach}, 2} = 2r$.
                    \item Tous les points ont au moins $K=2$ voisins à distance $2r$ (leurs cercles de rayon $r$ se touchent).
                    \item Les $6$ points fusionnent \textbf{simultanément} en un unique composant.
                    \item Incapacité à séparer les deux triangles du pont de liaison $C\text{-}D$.
                \end{itemize}
            }
            \only<2>{
                \textbf{La vraie hiérarchie $2$-NN} :
                \begin{itemize}
                    \setlength{\itemsep}{6pt}
                    \item À la résolution $r' = \frac{2\sqrt{3}}{3}r \approx 1.15r$, les boules s'intersectent trois à trois aux centres des triangles (points rouges).
                    \item Les deux triangles gauche et droit forment chacun un cluster séparé.
                    \item Le segment central $C\text{-}D$ reste \textbf{isolé} (pas de 3-intersection pour le connecter).
                    \item Permet de séparer les deux structures de forte densité du pont de bruit.
                \end{itemize}
            }
        \end{column}
    \end{columns}
