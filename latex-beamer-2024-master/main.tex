    ...
    \begin{columns}[c, onlytextwidth]
        \begin{column}{0.54\textwidth}
            \begin{itemize}
                ...
                \end{itemize}
                \item \textbf{Intérêt algorithmique} :
                \begin{itemize}
                    \item Support optimal pour calculer le MST en $\mathcal{O}(n \log n)$ dans $\mathbb{R}^2$.
                \end{itemize}
            \end{itemize}
        \end{column}
    ...
    \begin{columns}[c, onlytextwidth]
        \begin{column}{0.54\textwidth}
            \begin{itemize}
                ...
                \begin{itemize}
                    \item Soit $\sigma$ un $K$-simplexe de rayon $r_\sigma = \rho(\sigma)$.
                    \item $\sigma$ est \textbf{$K$-séparant} si la connectivité des $K$-polyèdres change à l'échelle $r_\sigma$ :
                    \[ \Gamma_K(\mathcal{X})_{< r_\sigma} \ \neq \ \Gamma_K(\mathcal{X})_{\le r_\sigma} \]
                    \item En pratique : il provoque une fusion dans la hiérarchie.
                \end{itemize}
                \item \textbf{Condition de \textsc{Gabriel}} :
                \begin{itemize}
                    \item $\sigma$ est de \textbf{\textsc{Gabriel}} si l'intérieur de sa plus petite boule englobante $\mathring{B}_\sigma$ est vide :
                    \[ \mathring{B}_\sigma \cap (\mathcal{X} \setminus \sigma) = \varnothing \]
                \end{itemize}
                \item \textbf{Théorème (Condition nécessaire)} :
                \begin{itemize}
                    \item \textbf{Tout simplexe $K$-séparant est de \textsc{Gabriel}}.
                    \item Support de recherche optimal : la \textbf{mosaïque de \textsc{Delaunay} d'ordre $K$}.
                \end{itemize}
            \end{itemize}
        \end{column}
        ...
        \begin{column}{0.44\textwidth}
            \begin{center}
                \begin{tikzpicture}[scale=1.1, >=latex]
                    ...
                    \coordinate (B) at (-1.1, -0.6);
                    \coordinate (C) at (1.1, -0.6);
                    
                    % Circumcircle of ABC
                    \draw[fill=inria-bleu!10, draw=inria-bleu, thick] (0,0) circle (1.2);
                    
                    \draw[line width=1.2pt, inria-rouge, fill=inria-rouge!15] (A) -- (B) -- (C) -- cycle;
                    
                    \fill[black] (A) circle (2pt) node[above] {$x_1$};
                    \fill[black] (B) circle (2pt) node[below left] {$x_2$};
                    \fill[black] (C) circle (2pt) node[below right] {$x_3$};
                    
                    \node[below, font=\scriptsize] at (0,-1.0) {\textbf{Triangle de \textsc{Gabriel} ($K=2$)}};
                \end{tikzpicture}
            \end{center}
        \end{column}
    \end{columns}
    ...
    \begin{columns}[c, onlytextwidth]
        \begin{column}{0.54\textwidth}
            ...
            \begin{itemize}
                ...
                \begin{itemize}
                    \item La recherche se restreint aux simplexes de la \textbf{mosaïque de \textsc{Delaunay} d'ordre $K$}.
                \end{itemize}
            \end{itemize}
        \end{column}
